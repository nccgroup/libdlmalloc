...
#
# Note libdmalloc_28x was primarily tested on 2.8.3, which is not the latest
# version. Testing was done on a version that was compiled with the MSPACE and
# FOOTER constants.
#
# See http://gee.cs.oswego.edu/pub/misc/malloc-2.8.3.c
...
class dl_helper:
    ...
    def __init__(self, size_sz=0):
        ...
        self.TREE_HDR_SZ       = 8 * self.SIZE_SZ
    ...
    def pinuse(self, p):
        ...
        return (p.head & self.PINUSE_BIT)

    def chunksize(self, p):
        "Get size, ignoring use bits"
        return (p.head & ~self.INUSE_BITS)
    ...
    def next_chunk(self, p):
        ...
        return (p.address + (p.size & ~self.INUSE_BITS))

    def prev_chunk(self, p):
        "Ptr to previous physical malloc_chunk"
        return (p.address - p.prev_foot)
    ...
    def cinuse(self, p):
        ...
        return (p.head & self.CINUSE_BIT)

    def set_cinuse(self, p):
        "set chunk as being inuse without otherwise disturbing"
        chunk = dl_chunk(self, (p.address + (p.size & ~self.INUSE_BITS)),
                            inuse=False)
        chunk.size |= self, self.CINUSE_BIT
        chunk.write()
    ...
    def is_end_chunk(self, p):
        if not self.cinuse(p) and not self.pinuse(p):
            return True
        return False
    ...
    def print_segment_chunks(self, seg=None, show_free=True, show_inuse=True, 
        ...
        while addr < (seg.base + seg.size):
            ...
            if sz != 0:
                if chunksz != sz:
                    continue
            if show_free != True and not self.dl.cinuse(chunk):
                continue
            if show_inuse != True and self.dl.cinuse(chunk):
                continue
...
class dl_chunk(dl_structure):
    ...
    def __init__(self, dl, addr=None, mem=None, size=None, inferior=None, 
        ...
        if mem == None:
            ...
            try:
                # TREE_HDR_SZ is based on SIZE_SZ already
                mem = self.inferior.read_memory(self.address, self.dl.TREE_HDR_SZ)
                # XXX - Technically if the last chunk is only 0x10 bytes, and
                # we read it as a TREE_HDR it will fail. Better way to do the
                # above
            except TypeError:
            ...
            if self.inuse:
                ...
                # XXX - This should test for FREE_HDR_SZ as well
                if (len(mem) != self.dl.TREE_HDR_SZ) and \
                                (len(mem) < self.dl.TREE_HDR_SZ):
        ...
        if self.dl.SIZE_SZ == 4:
            (self.prev_foot,
            self.head) = struct.unpack_from("<II", mem, 0x0)
...
#                self.data = struct.unpack_from("<%dI" % real_size, mem, self.dl.INUSE_HDR_SZ)
        ...
        if not self.inuse:
            ...
            if real_size > self.dl.MAX_SMALL_SIZE:
                ...
                if self.dl.SIZE_SZ == 4:
                    ...
                    self.right, \
                    self.parent,\
                    self.bindex) = struct.unpack_from("<4I", mem, self.dl.FREE_HDR_SZ)
                elif self.dl.SIZE_SZ == 8:
                    (self.left,  \
                    self.right,  \
                    self.parent, \
                    self.bindex ) = struct.unpack_from("<4Q", mem, self.dl.FREE_HDR_SZ)
            else:
    ...
    def __str__(self):
        if self.prev_foot == 0 and self.head == 0:
            return ""
        # CINUSE chunk
        elif self.inuse:
            ...
            mc += "{"
            mc += "\n{:11} = ".format("prev_foot")
            mc += "{:#x}".format(self.prev_foot)
            mc += "\n{:11} = ".format("size")
            mc += "{:#x}".format(self.head & ~self.dl.INUSE_BITS)
            if self.cinuse_bit == 1 or self.pinuse_bit == 1:
                mc += " ("
                if self.cinuse_bit == 1:
                    mc += "CINUSE|"
